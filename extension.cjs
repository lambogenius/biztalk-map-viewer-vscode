...
const viewType = 'biztalkMapViewer.btmEditor';
...
function activate(context) {
  const provider = vscode.window.registerCustomEditorProvider(viewType, {
    resolveCustomTextEditor(document, panel) {
      ...
      const send = () => panel.webview.postMessage({
        type: 'openMap',
      ...
      const changes = vscode.workspace.onDidChangeTextDocument(event => {
        if (event.document.uri.toString() === document.uri.toString()) void send();
      });
      panel.onDidDispose(() => { messages.dispose(); changes.dispose(); });
    },
  }, { webviewOptions: { retainContextWhenHidden: true } });

  const command = vscode.commands.registerCommand('biztalkMapViewer.open', async uri => {
    const target = uri || (await vscode.window.showOpenDialog({
      canSelectMany: false,
      filters: { 'BizTalk maps': ['btm'] },
    }))?.[0];
